...
Atl_15min_year_data = pd.read_csv('Atlanta/Atlanta_15_minute_timeseries_data.csv', low_memory=False)
Atl_dwell = pd.read_csv('Atlanta/Atlanta_dwellingunits.csv')

Atl_dwelling_units = Atl_dwell.iloc[:, 3].sum()

# Load CSV without automatic parsing
# Convert the timestamp column explicitly
Atl_15min_year_data["Timestamp (EST)"] = pd.to_datetime(
    Atl_15min_year_data["Timestamp (EST)"], errors="coerce"
)
# Set as index
Atl_15min_year_data.set_index("Timestamp (EST)", inplace=True)
specific_day = Atl_15min_year_data.loc["2018-01-02"]
print(specific_day)
# Remove timezone information
Atl_15min_year_data.index = Atl_15min_year_data.index.tz_localize(None)
# Extract Year-Month
Atl_15min_year_data["Year-Month"] = Atl_15min_year_data.index.to_period("M")
electricity_columns = [col for col in Atl_15min_year_data.columns if 'electricity' in col.lower()]
...
Atl_15min_year_data.rename(columns={col: clean_column_name(col) for col in Atl_15min_year_data.columns}, inplace=True)

# Print to check
monthly_usage_total = Atl_15min_year_data.groupby("Year-Month").sum()
monthly_usage = monthly_usage_total / Atl_dwelling_units
...
color_order = [colors[2], colors[0], colors[4], colors[3], colors[1], colors[5]]  # Custom order
monthly_usage_top10.plot(kind="bar", figsize=(12, 6), width=0.8,color = color_order)
plt.xlabel("Month")
plt.ylabel("Electricity Usage (kWh)")
plt.title("Atlanta Monthly Electricity Usage per Home 2018")
...
# Print to check
daily_usage = specific_day / Atl_dwelling_units
...
color_order_new = [colors[3], colors[4], colors[7], colors[1], colors[9]]  # Custom order
daily_usage_top10.plot(kind="bar", figsize=(10, 5), width=0.8,color = color_order_new)

plt.xlabel("Hour of the Day")
plt.ylabel("Electricity Usage (kWh)")
plt.title("Atlanta Hourly Electricity Usage on January 2nd, 2018")
