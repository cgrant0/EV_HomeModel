...
SD_15min_year_data = pd.read_csv('/Users/zackburkhardt/Documents/Spring 2025/Hyundai Research/Data/Best Data/San Diego/SanDiego_15_minute_timeseries_data.csv', low_memory=False)
SD_dwell = pd.read_csv('/Users/zackburkhardt/Documents/Spring 2025/Hyundai Research/Data/Best Data/San Diego/SanDiego_dwellingunits.csv')

SD_dwelling_units = SD_dwell.iloc[:, 3].sum()


# Load CSV without automatic parsing
# Convert the timestamp column explicitly
SD_15min_year_data["Timestamp (EST)"] = pd.to_datetime(
    SD_15min_year_data["Timestamp (EST)"], errors="coerce"
)
# Set as index
SD_15min_year_data.set_index("Timestamp (EST)", inplace=True)
specific_day = SD_15min_year_data.loc["2018-01-02"]
# Remove timezone information
SD_15min_year_data.index = SD_15min_year_data.index.tz_localize(None)
# Extract Year-Month
SD_15min_year_data["Year-Month"] = SD_15min_year_data.index.to_period("M")
electricity_columns = [col for col in SD_15min_year_data.columns if 'electricity' in col.lower()]
...
SD_15min_year_data.rename(columns={col: clean_column_name(col) for col in SD_15min_year_data.columns}, inplace=True)

# Print to check
monthly_usage_total = SD_15min_year_data.groupby("Year-Month").sum()
monthly_usage = monthly_usage_total / SD_dwelling_units
...
color_order = [colors[2], colors[4], colors[0], colors[1], colors[3], colors[5]]  # Custom order
monthly_usage_top10.plot(kind="bar", figsize=(12, 6), width=0.8,color = color_order)
print(plt.cm.tab10.colors)
plt.xlabel("Month")
plt.ylabel("Electricity Usage (kWh)")
plt.title("San Diego Monthly Electricity Usage per Home 2018")
...
# Print to check
daily_usage = specific_day / SD_dwelling_units
...
color_order_new = [colors[4], colors[3], colors[1], colors[0], colors[6]]  # Custom order
daily_usage_top10.plot(kind="bar", figsize=(10, 5), width=0.8,color = color_order_new)

plt.xlabel("Hour of the Day")
plt.ylabel("Electricity Usage (kWh)")
plt.title("San Diego Hourly Electricity Usage on January 2nd, 2018")
